  ...
  class="flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 hover:text-gray-900 transition-colors duration-150">
    ...
    <img class="h-auto max-h-16 w-auto object-contain" src="{{ asset('images/site_logo.png') }}" alt="EthniCart Logo" />
                    ...
                    <div class="hidden md:flex flex-1 max-w-2xl mx-8 relative">
                        <form method="GET" action="{{ url('/search') }}" class="w-full" id="searchForm">
                        ...
                        <div id="searchSuggestions" class="absolute top-full left-0 right-0 mt-2 bg-white rounded-lg shadow-xl border border-gray-200 max-h-96 overflow-y-auto z-50 hidden">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (searchInput) {
        ...
        // Reopen suggestions when focusing on input with existing value
    ...
    function displaySuggestions(products, query) {
        ...
        html += `
            <div class="border-t border-gray-200 mt-2">
                <button onclick="document.getElementById('searchForm').submit()" class="w-full px-4 py-3 text-center text-sm font-medium text-primary hover:bg-primary/5 transition-colors">
